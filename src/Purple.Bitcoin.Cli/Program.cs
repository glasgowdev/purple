        ...
        public static void Main(string[] args)
        {
            try
            {
                ...
                if (optionList.Contains("-help") || optionList.Contains("--help") || string.IsNullOrWhiteSpace(command))
                {
                    ...
                    builder.AppendLine(" dotnet run <Purple.Bitcoin.Cli/Purple.Bitcoin.Cli.dll> [network-name] [options] <command> [arguments]");
                    builder.AppendLine();
                    builder.AppendLine("Command line arguments:");
                    builder.AppendLine();
                    builder.AppendLine("[network-name]                     Name of the network - e.g. \"Purple\", \"Purplemain\", \"Purpletest\", \"bitcoinmain\", \"bitcointest\".");
                    ...
                    builder.AppendLine("-rpcport=<port>                    Connect to JSON-RPC on <port> (default for Purple: 26174 or default for Bitcoin: 8332)");
                    ...
                    builder.AppendLine();
                    builder.AppendLine("dotnet run Purple Wallet/history WalletName=testwallet - Lists all the historical transactions of the wallet called 'testwallet'.");
                    builder.AppendLine("dotnet run Purple getinfo -rpcuser=Purpletestuser -rpcpassword=Purpletestpassword -rpcconnect=127.0.0.3 -rpcport=26174 - Displays general information about the Purple node on the 127.0.0.3:26174, authenticating with the RPC specified user.");
