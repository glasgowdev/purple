...
namespace NBitcoin.Tests
{
    public class pos_ProtocolTests
    {
        ...
        public static bool noClient = !Process.GetProcesses().Any(p => p.ProcessName.Contains("Purple"));
        ...
        public void CanCancelConnection()
        {
            ...
            using (var builder = NodeBuilderPurple.Create())
            {
                ...
                try
                {
                    var client = Node.Connect(Network.PurpleMain, "127.0.0.1:" + node.ProtocolPort.ToString(), new NodeConnectionParameters()
                    {
        ...
        public void CanGetTransactionsFromMemPool()
        {
            ...
            using (var builder = NodeBuilderPurple.Create())
            {
                ...
                //node.Start();
                //node.Generate(101);
                //node.CreateRPCClient().SendToAddress(new Key().PubKey.GetAddress(Network.PurpleMain), Money.Coins(1.0m));
        ...
        public void CanDownloadBlock()
        {
            ...
            using (var builder = NodeBuilderPurple.Create())
            {
                ...
                {
                    Hash = Network.PurpleMain.GenesisHash,
