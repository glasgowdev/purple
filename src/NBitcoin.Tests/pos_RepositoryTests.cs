...
namespace NBitcoin.Tests
{
    public class pos_RepositoryTests
    {
        ...
        {
            int count = 0;
            foreach (StoredBlock stored in StoredBlock.EnumerateFile(TestDataLocations.DataBlockFolder("blk0001.dat"), network:Network.PurpleMain))
            {
                Assert.True(stored.Item.Check(Network.PurpleMain.Consensus));
                count++;
            }
            Assert.Equal(2000, count);

            count = 0;
            var twoLast = StoredBlock.EnumerateFile(TestDataLocations.DataBlockFolder("blk0001.dat"), network: Network.PurpleMain).Skip(1998).ToList();
            foreach(StoredBlock stored in StoredBlock.EnumerateFile(TestDataLocations.DataBlockFolder("blk0001.dat"), network: Network.PurpleMain, range: new DiskBlockPosRange(twoLast[0].BlockPosition)))
                count++;
        ...
        {
            var store = new BlockStore(TestDataLocations.DataFolder(@"blocks"), Network.PurpleMain);
        ...
        public void CanIndexBlock()
        {
            ...
            foreach (StoredBlock block in StoredBlock.EnumerateFile(TestDataLocations.DataBlockFolder("blk0001.dat"), network:Network.PurpleMain).Take(50))
        ...
        public void CanStoreBlocksInMultipleFiles()
        {
            ...
            var allBlocks = StoredBlock.EnumerateFile(TestDataLocations.DataBlockFolder("blk0001.dat"), network: Network.PurpleMain).Take(10).ToList();
            foreach (StoredBlock s in allBlocks)
                store.Append(s.Item);
        ...
        {
            var source = new BlockStore(TestDataLocations.DataFolder(@"blocks"), Network.PurpleMain);
        ...
        {
            BlockUndoStore src = new BlockUndoStore(TestDataLocations.DataFolder(@"blocks"), Network.PurpleMain);
        ...
        public void CanStoreInBlockRepository()
        {
            ...
            StoredBlock firstblk1 = StoredBlock.EnumerateFile(TestDataLocations.DataBlockFolder("blk0001.dat"), network: Network.PurpleMain).First();
        ...
        {
            var blk0 = StoredBlock.EnumerateFile(TestDataLocations.DataBlockFolder("blk0001.dat"), (uint)1, network: Network.PurpleMain).ToList();
            var blk1 = StoredBlock.EnumerateFile(TestDataLocations.DataBlockFolder("blk0002.dat"), (uint)2, network: Network.PurpleMain).ToList();

            int count = 0;
            foreach (StoredBlock stored in StoredBlock.EnumerateFolder(TestDataLocations.DataFolder(@"blocks"), network: Network.PurpleMain))
            {
                if (count == 0)
            ...
            Assert.Equal(2, StoredBlock.EnumerateFolder(TestDataLocations.DataFolder(@"blocks"), 
                new DiskBlockPosRange(blk1[1998].BlockPosition), network: Network.PurpleMain).Count());

            Assert.Equal(2002, StoredBlock.EnumerateFolder(TestDataLocations.DataFolder(@"blocks"), 
                new DiskBlockPosRange(blk0[1998].BlockPosition), network: Network.PurpleMain).Count());

            Assert.Equal(4, StoredBlock.EnumerateFolder(TestDataLocations.DataFolder(@"blocks"), 
                new DiskBlockPosRange(blk0[1998].BlockPosition, blk1[2].BlockPosition), network: Network.PurpleMain).Count());

            Assert.Equal(4, StoredBlock.EnumerateFolder(TestDataLocations.DataFolder(@"blocks"), 
                new DiskBlockPosRange(blk0[30].BlockPosition, blk0[34].BlockPosition), network: Network.PurpleMain).Count());
        }
        ...
        //[Trait("UnitTest", "UnitTest")]
        /*
        public void EnumerateRawPurpleBlockcahinAndWriteResultsToFile()
        {
        ...
        */
        ...
        //[Trait("UnitTest", "UnitTest")]
        /*
        public void EnumerateRawPurpleBlockcahinAndCompareWithRpcServer()
        {
            // this is a long running tests and take a few hours to complete.
            // the tests requires a Purple full node rpc server to be running
            // validate all blocks with the full node rpc server
            // at the time of writing the test the Purple blockchain was just beyond the 90k mark
            ...
            var client = new RPCClient(new NetworkCredential("rpcuser", "rpcpassword"), new Uri("http://127.0.0.1:5000"), Network.PurpleMain);
        ...
        */
        ...
        public void EnumerateAndValidateAllBlocks()
        {
            ...
            var store = new BlockStore(TestDataLocations.BlockFolderLocation, Network.PurpleMain);
            foreach (StoredBlock block in store.EnumerateFolder())
            {
                uint256 hash = block.Item.GetHash();
                listAll.Add(hash.ToString(), block);
                Assert.True(block.Item.Check(Network.PurpleMain.Consensus));
            }
            ...
            uint256 genesis = Network.PurpleMain.GetGenesis().GetHash();
        ...
        public void EnumerateAndCheckTipBlock()
        {
            ...
            var block100K = uint256.Parse("af380a53467b70bc5d1ee61441586398a0a5907bb4fad7855442575483effa54");
            ConcurrentChain chain = store.GetPurpleChain();
        ...
        public void IndexTheFullChain()
        {
            ...
            foreach (ChainedBlock item in chain.ToEnumerable(false))
            {
                ...
                Assert.True(BlockValidator.CheckBlock(Network.PurpleMain.Consensus, block));
            }            
        }
        ...
        public void CheckBlockProofOfStake()
        {
            ...
            var mainStore = new BlockStore(TestDataLocations.BlockFolderLocation, Network.PurpleMain);
            ...
            if (!pos_RPCClientTests.noClient)
                client = new RPCClient(new NetworkCredential("rpcuser", "rpcpassword"),
                    new Uri("http://127.0.0.1:" + Network.PurpleMain.RPCPort), Network.PurpleMain);

            var stakeChain = new MemoryStakeChain(Network.PurpleMain);
