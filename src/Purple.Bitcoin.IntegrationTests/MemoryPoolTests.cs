...
using Purple.Bitcoin.Features.MemoryPool.Fee;
    ...
    public class MemoryPoolTests
    {
        ...
        public void AddToMempool()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                PurpleNodeSync.GeneratePurple(105); // coinbase maturity = 100
                ...
                var block = PurpleNodeSync.FullNode.BlockStoreManager().BlockRepository.GetAsync(PurpleNodeSync.FullNode.Chain.GetBlock(4).HashBlock).Result;
                var prevTrx = block.Transactions.First();
                var dest = new BitcoinSecret(new Key(), PurpleNodeSync.FullNode.Network);

                Transaction tx = new Transaction();
                tx.AddInput(new TxIn(new OutPoint(prevTrx.GetHash(), 0), PayToPubkeyHashTemplate.Instance.GenerateScriptPubKey(PurpleNodeSync.MinerSecret.PubKey)));
                tx.AddOutput(new TxOut("25", dest.PubKey.Hash));
                tx.AddOutput(new TxOut("24", new Key().PubKey.Hash)); // 1 btc fee
                tx.Sign(PurpleNodeSync.MinerSecret, false);

                PurpleNodeSync.Broadcast(tx);

                TestHelper.WaitLoop(() => PurpleNodeSync.CreateRPCClient().GetRawMempool().Length == 1);
            }
        }
        ...
        public void AddToMempoolTrxSpendingTwoOutputFromSameTrx()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                var block = PurpleNodeSync.FullNode.BlockStoreManager().BlockRepository.GetAsync(PurpleNodeSync.FullNode.Chain.GetBlock(4).HashBlock).Result;
                var prevTrx = block.Transactions.First();
                var dest1 = new BitcoinSecret(new Key(), PurpleNodeSync.FullNode.Network);
                var dest2 = new BitcoinSecret(new Key(), PurpleNodeSync.FullNode.Network);

                Transaction parentTx = new Transaction();
                parentTx.AddInput(new TxIn(new OutPoint(prevTrx.GetHash(), 0), PayToPubkeyHashTemplate.Instance.GenerateScriptPubKey(PurpleNodeSync.MinerSecret.PubKey)));
                parentTx.AddOutput(new TxOut("25", dest1.PubKey.Hash));
                parentTx.AddOutput(new TxOut("24", dest2.PubKey.Hash)); // 1 btc fee
                parentTx.Sign(PurpleNodeSync.MinerSecret, false);
                PurpleNodeSync.Broadcast(parentTx);
                // wiat for the trx to enter the pool
                TestHelper.WaitLoop(() => PurpleNodeSync.CreateRPCClient().GetRawMempool().Length == 1);
                // mine the transactions in the mempool
                PurpleNodeSync.GeneratePurple(1, PurpleNodeSync.FullNode.MempoolManager().InfoAllAsync().Result.Select(s => s.Trx).ToList());
                TestHelper.WaitLoop(() => PurpleNodeSync.CreateRPCClient().GetRawMempool().Length == 0);
                ...
                PurpleNodeSync.Broadcast(signed);
                TestHelper.WaitLoop(() => PurpleNodeSync.CreateRPCClient().GetRawMempool().Length == 1);
            }
        }
        ...
        public void MempoolReceiveFromManyNodes()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                PurpleNodeSync.SetDummyMinerSecret(new BitcoinSecret(new Key(), PurpleNodeSync.FullNode.Network));
                PurpleNodeSync.GeneratePurple(201); // coinbase maturity = 100
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.ConsensusLoop().Tip.HashBlock == PurpleNodeSync.FullNode.Chain.Tip.HashBlock);
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.HighestPersistedBlock().HashBlock == PurpleNodeSync.FullNode.Chain.Tip.HashBlock);
                ...
                {
                    var block = PurpleNodeSync.FullNode.BlockStoreManager().BlockRepository.GetAsync(PurpleNodeSync.FullNode.Chain.GetBlock(index).HashBlock).Result;
                    var prevTrx = block.Transactions.First();
                    var dest = new BitcoinSecret(new Key(), PurpleNodeSync.FullNode.Network);
                ...
                {
                    PurpleNodeSync.Broadcast(transaction);
                });

                TestHelper.WaitLoop(() => PurpleNodeSync.CreateRPCClient().GetRawMempool().Length == 100);
            }
        }
        ...
        public void TxMempoolBlockDoublespend()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                builder.StartAll();
                PurpleNodeSync.NotInIBD();
                PurpleNodeSync.FullNode.Settings.RequireStandard = true; // make sure to test standard tx

                PurpleNodeSync.SetDummyMinerSecret(new BitcoinSecret(new Key(), PurpleNodeSync.FullNode.Network));
                PurpleNodeSync.GeneratePurple(100); // coinbase maturity = 100
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.ConsensusLoop().Tip.HashBlock == PurpleNodeSync.FullNode.Chain.Tip.HashBlock);
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.HighestPersistedBlock().HashBlock == PurpleNodeSync.FullNode.Chain.Tip.HashBlock);
                ...
                var scriptPubKey = PayToPubkeyHashTemplate.Instance.GenerateScriptPubKey(PurpleNodeSync.MinerSecret.PubKey);
                var genBlock = PurpleNodeSync.FullNode.BlockStoreManager().BlockRepository.GetAsync(PurpleNodeSync.FullNode.Chain.GetBlock(1).HashBlock).Result;
                ...
                foreach (var index in Enumerable.Range(1, 2))
                {
                    ...
                    // Sign:
                    trx.Sign(PurpleNodeSync.MinerSecret, false);
                    spends.Add(trx);
                }

                // Test 1: block with both of those transactions should be rejected.
                var block = PurpleNodeSync.GeneratePurple(1, spends).Single();
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.ConsensusLoop().Tip.HashBlock == PurpleNodeSync.FullNode.Chain.Tip.HashBlock);
                Assert.True(PurpleNodeSync.FullNode.Chain.Tip.HashBlock != block.GetHash());

                // Test 2: ... and should be rejected if spend1 is in the memory pool
                Assert.True(PurpleNodeSync.AddToPurpleMempool(spends[0]));
                block = PurpleNodeSync.GeneratePurple(1, spends).Single();
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.ConsensusLoop().Tip.HashBlock == PurpleNodeSync.FullNode.Chain.Tip.HashBlock);
                Assert.True(PurpleNodeSync.FullNode.Chain.Tip.HashBlock != block.GetHash());
                PurpleNodeSync.FullNode.MempoolManager().Clear().Wait();

                // Test 3: ... and should be rejected if spend2 is in the memory pool
                Assert.True(PurpleNodeSync.AddToPurpleMempool(spends[1]));
                block = PurpleNodeSync.GeneratePurple(1, spends).Single();
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.ConsensusLoop().Tip.HashBlock == PurpleNodeSync.FullNode.Chain.Tip.HashBlock);
                Assert.True(PurpleNodeSync.FullNode.Chain.Tip.HashBlock != block.GetHash());
                PurpleNodeSync.FullNode.MempoolManager().Clear().Wait();
                ...
                Assert.True(PurpleNodeSync.AddToPurpleMempool(spends[1]));
                block = PurpleNodeSync.GeneratePurple(1, oneSpend).Single();
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.ConsensusLoop().Tip.HashBlock == PurpleNodeSync.FullNode.Chain.Tip.HashBlock);
                Assert.True(PurpleNodeSync.FullNode.Chain.Tip.HashBlock == block.GetHash());

                // spends[1] should have been removed from the mempool when the
                // block with spends[0] is accepted:
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.MempoolManager().MempoolSize().Result == 0);
            }
        }
        ...
        public void TxMempoolMapOrphans()
        {
            ...
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                PurpleNode.SetDummyMinerSecret(new BitcoinSecret(new Key(), PurpleNode.FullNode.Network));
                ...
                for (ulong i = 0; i < 50; i++)
                {
                    ...
                    tx.AddOutput(new TxOut(new Money(1 * Money.CENT), PurpleNode.MinerSecret.ScriptPubKey));

                    PurpleNode.FullNode.MempoolManager().Orphans.AddOrphanTx(i, tx).Wait();
                }

                Assert.Equal(50, PurpleNode.FullNode.MempoolManager().Orphans.OrphansList().Count);

                // ... and 50 that depend on other orphans:
                for (ulong i = 0; i < 50; i++)
                {
                    var txPrev = PurpleNode.FullNode.MempoolManager().Orphans.OrphansList().ElementAt(rand.Next(PurpleNode.FullNode.MempoolManager().Orphans.OrphansList().Count));

                    Transaction tx = new Transaction();
                    tx.AddInput(new TxIn(new OutPoint(txPrev.Tx.GetHash(), 0), new Script(OpcodeType.OP_1)));
                    tx.AddOutput(new TxOut(new Money((1 + i + 100) * Money.CENT), PurpleNode.MinerSecret.ScriptPubKey));
                    PurpleNode.FullNode.MempoolManager().Orphans.AddOrphanTx(i, tx).Wait();
                }

                Assert.Equal(100, PurpleNode.FullNode.MempoolManager().Orphans.OrphansList().Count);
                ...
                for (ulong i = 0; i < 10; i++)
                {
                    ...
                    Assert.False(PurpleNode.FullNode.MempoolManager().Orphans.AddOrphanTx(i, tx).Result);
                }
                ...
                for (ulong i = 0; i < 3; i++)
                {
                    var sizeBefore = PurpleNode.FullNode.MempoolManager().Orphans.OrphansList().Count;
                    PurpleNode.FullNode.MempoolManager().Orphans.EraseOrphansFor(i).Wait();
                    Assert.True(PurpleNode.FullNode.MempoolManager().Orphans.OrphansList().Count < sizeBefore);
                }

                // Test LimitOrphanTxSize() function:
                PurpleNode.FullNode.MempoolManager().Orphans.LimitOrphanTxSizeAsync(40).Wait();
                Assert.True(PurpleNode.FullNode.MempoolManager().Orphans.OrphansList().Count <= 40);
                PurpleNode.FullNode.MempoolManager().Orphans.LimitOrphanTxSizeAsync(10).Wait();
                Assert.True(PurpleNode.FullNode.MempoolManager().Orphans.OrphansList().Count <= 10);
                PurpleNode.FullNode.MempoolManager().Orphans.LimitOrphanTxSizeAsync(0).Wait();
                Assert.True(!PurpleNode.FullNode.MempoolManager().Orphans.OrphansList().Any());
            }
        }
        ...
        public void MempoolAddNodeWithOrphans()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                PurpleNodeSync.SetDummyMinerSecret(new BitcoinSecret(new Key(), PurpleNodeSync.FullNode.Network));
                PurpleNodeSync.GeneratePurple(101); // coinbase maturity = 100
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.ConsensusLoop().Tip.HashBlock == PurpleNodeSync.FullNode.Chain.Tip.HashBlock);
                ...
                var block = PurpleNodeSync.FullNode.BlockStoreManager().BlockRepository.GetAsync(PurpleNodeSync.FullNode.Chain.GetBlock(1).HashBlock).Result;
                var prevTrx = block.Transactions.First();
                var dest = new BitcoinSecret(new Key(), PurpleNodeSync.FullNode.Network);
                ...
                txOrphan.Sign(key.GetBitcoinSecret(PurpleNodeSync.FullNode.Network), false);

                // broadcast the orphan
                PurpleNodeSync.Broadcast(txOrphan);
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.MempoolManager().Orphans.OrphansList().Count == 1);
                // broadcast the parent
                PurpleNodeSync.Broadcast(tx);
                TestHelper.WaitLoop(() => PurpleNodeSync.FullNode.MempoolManager().Orphans.OrphansList().Count == 0);
                // wait for orphan to get in the pool
                TestHelper.WaitLoop(() => PurpleNodeSync.CreateRPCClient().GetRawMempool().Length == 2);
            }
        }
        ...
        public void MempoolSyncTransactions()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                PurpleNodeSync.SetDummyMinerSecret(new BitcoinSecret(new Key(), PurpleNodeSync.FullNode.Network));
                PurpleNodeSync.GeneratePurpleWithMiner(105); // coinbase maturity = 100
                ...
                TestHelper.WaitLoop(() => TestHelper.AreNodesSynced(PurpleNode1, PurpleNodeSync));
                TestHelper.WaitLoop(() => TestHelper.AreNodesSynced(PurpleNode2, PurpleNodeSync));
                ...
                TestHelper.WaitLoop(() => PurpleNodeSync.CreateRPCClient().GetRawMempool().Length == 5);

                // the full node should be connected to both nodes
                Assert.True(PurpleNodeSync.FullNode.ConnectionManager.ConnectedPeers.Count() >= 2);

                // reset the trickle timer on the full node that has the transactions in the pool
                foreach (var node in PurpleNodeSync.FullNode.ConnectionManager.ConnectedPeers) node.Behavior<MempoolBehavior>().NextInvSend = 0;

                TestHelper.WaitLoop(() => PurpleNode1.CreateRPCClient().GetRawMempool().Length == 5);
                TestHelper.WaitLoop(() => PurpleNode2.CreateRPCClient().GetRawMempool().Length == 5);

                // mine the transactions in the mempool
                PurpleNodeSync.GeneratePurpleWithMiner(1);
                TestHelper.WaitLoop(() => PurpleNodeSync.CreateRPCClient().GetRawMempool().Length == 0);
                ...
                TestHelper.WaitLoop(() => PurpleNode1.CreateRPCClient().GetRawMempool().Length == 0);
                TestHelper.WaitLoop(() => PurpleNode2.CreateRPCClient().GetRawMempool().Length == 0);
            }
        }
    }
