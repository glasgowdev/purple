...
using static Purple.Bitcoin.BlockPulling.BlockPuller;
...
namespace Purple.Bitcoin.IntegrationTests.EnvironmentMockUpHelpers
{
    public class CoreNode
    {
        ...
        public FullNode FullNode
        {
            get
            {
                if (this.runner is PurpleBitcoinPosRunner)
                    return ((PurpleBitcoinPosRunner)this.runner).FullNode;

                return ((PurpleBitcoinPowRunner)this.runner).FullNode;
            }
        }
        ...
        public bool AddToPurpleMempool(Transaction trx)
        {
            var fullNode = (this.runner as PurpleBitcoinPowRunner).FullNode;
            var state = new MempoolValidationState(true);

            return fullNode.MempoolManager().Validator.AcceptToMemoryPool(state, trx).Result;
        }

        public List<uint256> GeneratePurpleWithMiner(int blockCount)
        {
        ...
        public Block[] GeneratePurple(int blockCount, List<Transaction> passedTransactions = null, bool broadcast = true)
        {
            var fullNode = (this.runner as PurpleBitcoinPowRunner).FullNode;
