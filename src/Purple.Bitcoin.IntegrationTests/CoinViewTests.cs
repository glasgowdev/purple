...
using Purple.Bitcoin.Base.Deployments;
using Purple.Bitcoin.Configuration;
using Purple.Bitcoin.Features.Consensus;
using Purple.Bitcoin.Features.Consensus.CoinViews;
    ...
    public class CoinViewTests
    {
        ...
        public void CanHandleReorgs()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                //Core1 discovers 10 blocks, sends to Purple
                var tip = coreNode1.FindBlock(10).Last();
                PurpleNode.CreateRPCClient().AddNode(coreNode1.Endpoint, true);
                TestHelper.WaitLoop(() => PurpleNode.CreateRPCClient().GetBestBlockHash() == coreNode1.CreateRPCClient().GetBestBlockHash());
                PurpleNode.CreateRPCClient().RemoveNode(coreNode1.Endpoint);

                //Core2 discovers 20 blocks, sends to Purple
                tip = coreNode2.FindBlock(20).Last();
                PurpleNode.CreateRPCClient().AddNode(coreNode2.Endpoint, true);
                TestHelper.WaitLoop(() => PurpleNode.CreateRPCClient().GetBestBlockHash() == coreNode2.CreateRPCClient().GetBestBlockHash());
                PurpleNode.CreateRPCClient().RemoveNode(coreNode2.Endpoint);
                ((CachedCoinView)PurpleNode.FullNode.CoinView()).FlushAsync().Wait();

                //Core1 discovers 30 blocks, sends to Purple
                tip = coreNode1.FindBlock(30).Last();
                PurpleNode.CreateRPCClient().AddNode(coreNode1.Endpoint, true);
                TestHelper.WaitLoop(() => PurpleNode.CreateRPCClient().GetBestBlockHash() == coreNode1.CreateRPCClient().GetBestBlockHash());
                PurpleNode.CreateRPCClient().RemoveNode(coreNode1.Endpoint);

                //Core2 discovers 50 blocks, sends to Purple
