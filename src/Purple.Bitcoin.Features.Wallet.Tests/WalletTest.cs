...
namespace Purple.Bitcoin.Features.Wallet.Tests
{
    public class WalletTest : WalletTestBase
    {
        ...
        public void GetAccountsByCoinTypeReturnsAccountsFromWalletByCoinType()
        {
            ...
            wallet.AccountsRoot.Add(CreateAccountRootWithHdAccountHavingAddresses("PurpleAccount", CoinType.Purple));
            wallet.AccountsRoot.Add(CreateAccountRootWithHdAccountHavingAddresses("BitcoinAccount", CoinType.Bitcoin));
            wallet.AccountsRoot.Add(CreateAccountRootWithHdAccountHavingAddresses("PurpleAccount2", CoinType.Purple));

            var result = wallet.GetAccountsByCoinType(CoinType.Purple);

            Assert.Equal(2, result.Count());
            Assert.Equal("PurpleAccount", result.ElementAt(0).Name);
            Assert.Equal("PurpleAccount2", result.ElementAt(1).Name);
        }
        ...
        {
            var wallet = new Wallet();
            var PurpleAccountRoot = CreateAccountRootWithHdAccountHavingAddresses("PurpleAccount", CoinType.Purple);
            var bitcoinAccountRoot = CreateAccountRootWithHdAccountHavingAddresses("BitcoinAccount", CoinType.Bitcoin);
            var PurpleAccountRoot2 = CreateAccountRootWithHdAccountHavingAddresses("PurpleAccount2", CoinType.Purple);
            ...
            PurpleAccountRoot.Accounts.ElementAt(0).InternalAddresses.ElementAt(0).Transactions.Add(transaction1);
            PurpleAccountRoot.Accounts.ElementAt(0).ExternalAddresses.ElementAt(0).Transactions.Add(transaction2);
            ...
            PurpleAccountRoot2.Accounts.ElementAt(0).InternalAddresses.ElementAt(0).Transactions.Add(transaction5);
            PurpleAccountRoot2.Accounts.ElementAt(0).ExternalAddresses.ElementAt(0).Transactions.Add(transaction6);

            wallet.AccountsRoot.Add(PurpleAccountRoot);
            wallet.AccountsRoot.Add(bitcoinAccountRoot);
            wallet.AccountsRoot.Add(PurpleAccountRoot2);

            var result = wallet.GetAllTransactionsByCoinType(CoinType.Purple).ToList();
        ...
        public void GetAllPubKeysByCoinTypeReturnsPubkeysFromWalletByCoinType()
        {
            ...
            var result = wallet.GetAllPubKeysByCoinType(CoinType.Purple).ToList();

            Assert.Equal(4, result.Count);
            Assert.Equal(PurpleAccountRoot.Accounts.ElementAt(0).ExternalAddresses.ElementAt(0).ScriptPubKey, result[0]);
            Assert.Equal(PurpleAccountRoot2.Accounts.ElementAt(0).ExternalAddresses.ElementAt(0).ScriptPubKey, result[1]);
            Assert.Equal(PurpleAccountRoot.Accounts.ElementAt(0).InternalAddresses.ElementAt(0).ScriptPubKey, result[2]);
            Assert.Equal(PurpleAccountRoot2.Accounts.ElementAt(0).InternalAddresses.ElementAt(0).ScriptPubKey, result[3]);
        }
